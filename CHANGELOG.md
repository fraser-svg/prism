...
### Added
- **Frontend tool awareness** — Prism now offers to generate standalone UI screens (landing pages, dashboards, signup flows) via Google Stitch before the build stage starts. If Stitch is unavailable, Prism falls back to building the screen as a regular visual task. No configuration needed beyond `prism: connect stitch`.
- New `"screen"` route hint for standalone UI screen tasks, separate from `"visual"` (component/style) tasks. Screen tasks are handled at Stage 2.5 via Stitch, not decomposed into build workers.
- `stitch_fallback` telemetry event for tracking when Stitch is unavailable.
- Updated route_hint documentation in `docs/architecture/script-contracts.md`.
