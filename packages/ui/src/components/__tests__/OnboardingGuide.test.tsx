...
describe("OnboardingGuide", () => {
  ...
  it("shows step 1 checkmark when hasClients is true", () => {
    render(<OnboardingGuide {...defaults} hasClients={true} />);
    const circles = screen.getAllByLabelText(/Step \d of 2/);
    const step1 = circles.find((el) => el.getAttribute("aria-label") === "Step 1 of 2")!;
  ...
  it("progress bar shows 50% with one step done", () => {
    render(<OnboardingGuide {...defaults} hasClients={true} />);
    const progress = screen.getByRole("progressbar");
