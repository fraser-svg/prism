...
VIOLATION=""
if echo "$RECENT" | grep -qi "run this in your terminal"; then
  VIOLATION="directed founder to run a command in their terminal"
elif echo "$RECENT" | grep -qi "open a new terminal"; then
  VIOLATION="directed founder to open a new terminal"
elif echo "$RECENT" | grep -qi "in another window"; then
  VIOLATION="directed founder to use another window"
elif echo "$RECENT" | grep -qi "you'll need to manually"; then
  VIOLATION="told founder to do something manually"
elif echo "$RECENT" | grep -qi "you need to install"; then
  VIOLATION="told founder to install something themselves"
elif echo "$RECENT" | grep -qi "please run the following"; then
